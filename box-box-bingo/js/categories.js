...
function catAccentColor(text) {
  ...
  if (t.includes('sprint')) return '#00d68f';                               
  if (t.includes('laps completed')) return '#c0c0c0';                       
...
function buildDriverPool(shuffledAll, categories) {
  ...
  const bench = shuffledAll.slice(POOL_SIZE); 

  for (const cat of categories) {
    if (pool.some(d => cat.matches(d))) continue; 
    ...
    if (victimIdx === -1) victimIdx = pool.length - 1; 
...
function generateCategories() {
  ...
  Object.entries(TEAM_DRIVERS).forEach(([team, ids]) => {
    ...
    if (valid.length >= 3) filteredTeams[team] = valid; 
  });
  ...
  Object.entries(TEAMMATE_DRIVERS).forEach(([pid, ids]) => {
    const valid = ids.filter(id => validIds.has(id));
    if (valid.length >= 3) filteredMates[pid] = valid; 
  });
